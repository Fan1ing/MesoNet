...
triple_csv_path = '/data/surface tension.csv'
...
def process_molecule_hg(smiles):
    ...
    functional_groups_smarts = {
        # ===== 疏水链 =====
        "long_alkyl_chain": "C",  # 长烷基链（C6及以上）

        "hydroxyl": "[OX2H]",  # 羟基（末端OH）
        "ether": "C-O-C",  # 一般醚，包括PEG段
        "ester": "C(=O)O",  # 酯键或酰氧键
        "carbonyl": "C=O",  # 羰基（通用C=O）
        "carboxylate": "C(=O)[O-]",  # 羧酸盐/羧酸根
        "sulfate_ester": "[OX2][SX4](=O)(=O)[O-]",  # 硫酸酯（R–O–SO3–）
        "sulfonate": "[SX4](=O)(=O)[O-]",  # 磺酸盐（R–SO3–）

        # ===== 含氮阳离子 =====
        "quaternary_ammonium": "[NX4+]",  # 季铵盐（R4N+）
        "zwitterion_ammonium_carboxylate": "[NX4+].C(=O)[O-]",  # 两性型季铵-羧酸

        # ===== 芳香族 =====
        "phenyl": "c1ccccc1",  # 苯环（含苄基结构）
...
def combine_molecules_hg_2(smiles1, smiles2, x1=None, x2=None,C=None):
    ...
    x1 = torch.tensor([x1,C], dtype=torch.float32)
    x2 = torch.tensor([x2,C], dtype=torch.float32)
    ...
    if (G1 + G2) > 0:
        # x_group_all 需要是 [G_total, Dg]，即便某个分子无基团也要是二维空张量
    ...
    # 交叉全局特征（跳过前4维：HBD/HBA/logP/TPSA）
    ...
    # 每个图节点的“节点属性”：这里就用浓度，给两种排列（和你原三组分风格一致）
...
def load_data_two(csv_path):
    ...
    # 单一标签 y（比如表面张力、或你 CSV 里的那一列）
    y = df['surface_tension'].tolist()   # 把列名改成你的真实标签列名
...
dataset = MoleculesDatasetTwo(root='CMC', smiles1=smiles1, smiles2=smiles2,
...
class AtomGroupBridgeFiLM(nn.Module):
    def __init__(self, atom_dim, group_dim, cond_dim, hidden=180,s2s_steps: int = 2):
        ...
        self.film_gamma = nn.Sequential(
            ...
            nn.Linear(group_dim+42, group_dim+40)
        )
        ...
        self.g_proj = nn.Linear(16, group_dim-80)
        # Set2Set 聚合（A->G, G->A 用两个实例，互不共享参数）
        self.s2s_a2g = Set2Set(80, processing_steps=s2s_steps)   # 输出 2*Dg
        self.merge_a2g = nn.Linear(group_dim+80, group_dim+40)            # 2*Dg -> Dg
...
class CrossMolGroupInter(nn.Module):
    ...
    def forward(self, xg_list, gb_list):
        ...
        attn_unsorted = x_flat[inv]  # [N_tok, H_in]                # [N_tok, H_in]
...
class MesoNet(nn.Module):
    def __init__(self, input_dim, edge_dim, hidden_dim, output_dim,
        ...
        self.attn_global_group= FeatureCrossAttention(dim_in_q=16, dim_in_kv=32, model_dim=32, num_heads=4)
        self.inter            = FeatureCrossAttention(dim_in_q=18, dim_in_kv=18, model_dim=32, num_heads=4)
        ...
        self.NCP2= CfC(164, AutoNCP(320,160), batch_first=True)
        ...
        self.global_conv = NNConv(hidden_dim*2+4, hidden_dim, nn.Sequential(
            nn.Linear(4, edge_hidden_dim), nn.ReLU(), nn.Dropout(p=0.3),
            nn.Linear(edge_hidden_dim , 324*hidden_dim)
        ), aggr='mean')
        ...
        self.set2set2 = Set2Set(3*hidden_dim+4 , processing_steps=2)
        self.setgroup = Set2Set(240, processing_steps=2)
        ...
        self.lstm_a2_1 = CfC(32, AutoNCP(68,32), batch_first=True)
        self.NCP= CfC(164, AutoNCP(320,160), batch_first=True)

        # FiLM 参数
        self.c1_gamma = nn.Sequential(nn.Linear(4, 160), nn.ReLU(), nn.Linear(160, 160))
        self.c1_beta  = nn.Sequential(nn.Linear(4, 160), nn.ReLU(), nn.Linear(160, 160))
        self.c2_gamma = nn.Sequential(nn.Linear(4, 160), nn.ReLU(), nn.Linear(160, 160))
        self.c2_beta  = nn.Sequential(nn.Linear(4, 160), nn.ReLU(), nn.Linear(160, 160))
        ...
        self.atom_group_bridge = AtomGroupBridgeFiLM(
            atom_dim=hidden_dim, group_dim=hidden_dim,cond_dim = 4, s2s_steps=2
        )
    ...
    def process_subgraph(self, data, mask, mol_id):
        ...
        x1 = subgraph_x[:, 0:41]
        atom_onehot = subgraph_x[:, :16].detach().cpu().numpy()
        atom_types = atom_onehot.argmax(axis=1)  # 每个原子所属元素类型索引

        x1 = self.a11(x1, subgraph_edge_index, subgraph_edge_attr)
        x1 = self.relu(x1)
        x2 = subgraph_x[:, 41:47]
        x3 = subgraph_x[:, 47:47+16]

        g  = subgraph_x[:, 47+16+4:47+16+4+18]
        G_ = subgraph_x[:, 47+16+4+18:]
        C_raw = torch.cat((g[:, 16:18],G_[:, 16:18]), dim=1)
    ...
    def forward(self, data):
        ...
        #expand_group = self.setgroup(torch.cat((cross_stack, global_node_attr), dim=1), batch_vec)
        #expand_group = torch.cat((expand_group, mix_feat), dim=1)
        expand_group = torch.cat((mix_feat,expand_group),dim=1)

        group_out = torch.cat((expanded_x[0::K],expanded_x[1::K],set2set_x, expand_group,group[0::K],group[1::K]), dim=1)

        #final_x = torch.cat((group_out), dim=1)

        output = self.fc(group_out)
        return output # 第二返回保持你的接口
