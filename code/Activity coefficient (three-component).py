...
def combine_molecules(smiles1, smiles2, smiles3=None, x1=None, x2=None, x3=None):
    """
    处理两个或三个分子的 SMILES 表示并组合成一个图，生成全局边连接
    """
    # 处理第一个分子
    ...
    global_edge_index = torch.tensor(
        [[0, 1, 0, 2, 1, 2],  # 从分子1连接到分子2和分子3
         [1, 0, 2, 0, 2, 1]],  # 从分子2连接到分子1和分子3
        dtype=torch.long
    )
    ...
    mask1[:graph1.x.size(0)] = True  # 分子1的节点设为True

    mask2 = torch.zeros(combined_x.size(0), dtype=torch.bool)
    mask2[graph1.x.size(0):graph1.x.size(0) + graph2.x.size(0)] = True  # 分子2的节点设为True

    mask3 = torch.zeros(combined_x.size(0), dtype=torch.bool)
    if graph3:
        mask3[graph1.x.size(0) + graph2.x.size(0):] = True  # 分子3的节点设为True
...
def load_data(triple_csv_path):
    ...
    smiles3_triple = triple_df['solv3_smiles'].tolist()  # 三分子 SMILES
...
triple_csv_path = '/home/ubuntu/三组分全.csv'
...
class MultiLevelGraphNetWithEdgeFeatures(nn.Module):
    ...
    def forward(self, data):
        ...
        expanded_x[0::3] = subgraph1_x  # 在第 0, 3, 6... 索引处放置 subgraph1_x
        expanded_x[1::3] = subgraph2_x  # 在第 1, 4, 7... 索引处放置 subgraph2_x
        expanded_x[2::3] = subgraph3_x  # 在第 2, 5, 8... 索引处放置 subgraph3_x
