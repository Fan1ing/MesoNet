...
dataset = MoleculesDataset(root='datanewFF', smiles1=smiles1, smiles2=smiles2, smiles3=smiles3, targets=targets, concentrations=concentrations)
print(len(dataset))
...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            # 生成一个掩码，去除目标值为0的样本
            mask = torch.abs(target) < threshold  # 只保留目标值不为0的样本
